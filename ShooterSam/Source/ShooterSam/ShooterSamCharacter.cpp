...
void AShooterSamCharacter::Shoot()
{
	...
	UE_LOG(LogTemp, Display, TEXT("Shoot Is Calling!"));
}
...
void AShooterSamCharacter::UpdateHUD()
{
	...
	if (PlayerController && PlayerController->HUDWidget)
	{
		if (Health <= 0)
		{
			PlayerController->HUDWidget->SetHealthBarPercent(0);
			return;
		}
...
void AShooterSamCharacter::OnDamageTaken(AActor* DamagedActor, float Damage, const UDamageType* DamageType, AController* InstigatedBy, AActor* DamageCauser)
{
	if (IsAlive && Damage > 0)
	{
		...
		if (Health <= 0)
		{
			AShooterSamGameMode* GameMode = Cast<AShooterSamGameMode>(UGameplayStatics::GetGameMode(GetWorld()));
			if (GameMode)
			{
